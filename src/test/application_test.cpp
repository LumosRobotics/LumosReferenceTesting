...
TEST_F(ApplicationTest, ControlSystemValidation)
{
    ...
    auto is_small_magnitude = [](const double &x)
    { return std::abs(x) < 0.1; };
...
TEST_F(ApplicationTest, PerformanceMetricsValidation)
{
    ...
    auto small_error = [](const double &error)
    { return error < 0.1; };
    ...
    auto large_error = [](const double &error)
    { return error > 0.2; };
...
TEST_F(ApplicationTest, SystemHealthMonitoring)
{
    ...
    auto signal_dropout = [](const double &x)
    ...
    auto control_active = [](const double &x)
    { return std::abs(x) > 0.1; };
